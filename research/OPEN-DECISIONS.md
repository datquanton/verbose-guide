...
| 24 | **Locate the price and book date behind TCX's `2.49x P/B`, or replace it — then re-run one division. ← NEW, escalated 02-Aug 09:53. Escalation trigger #4 fired: a derived estimate became a filed actual.** | **A corrected denominator moved a KILL CRITERION from 24.5% away to 3.8% away with no price movement.** TCBS's filed end-Q2 equity is **₫45,782bn** (reported ratio 1.13×); the 31-Jul derivation used **~₫33,018bn** and **1.56×** — **39% low on equity**. Three figures for one quantity coexisted in this repo: `margin_equity_pct` **0.98**, the log's **1.56×**, the filed **1.125×**. **The capacity conclusion is withdrawn in TCX's favour** — headroom is **₫40,064bn, not ₫14,500bn**, 56% of cap drawn, not 78%, and the constrained brokers are **competitors** (HSC, KBSV, Phú Hưng all past 190%). **But the valuation side cuts the other way.** TCX has **2,311,308,021 shares** (never on file), so `price × shares` = **₫94,995bn** against `pe_ttm × npat_ttm` = **₫82,215bn** — **15.5% apart**, meaning `pe_ttm` 20.3 or `npat_ttm` 4,050 is wrong. **And `valuation.TCX.evidence` names the multiple as THE risk at 2.49×, which cannot be reproduced from any combination of file inputs and filed equity.** Reproducible P/B is **2.075×**; 2.49× needs a price of ~₫49,322 (above the ₫46,800 listing reference) or a book of ~₫38,151bn (below filed). **`decide.py`'s TCX kill criterion is "P/B < 2.0x" — trigger price ₫39,616 against a file price of ₫41,100.** **NOT declared fired**, because ₫41,100 is stale and already logged as a **trough**; if TCX traded up the distance is larger. **The point is that the distance was mismeasured by ~6×, and the input that settles it is item 2.** Note the criterion says *"on unchanged earnings"*: **equity grew 3.8% YTD, so the denominator rises and pulls P/B toward the trigger at a flat price.** Applied: confidence 0.80 → 0.75, evidence restated. `exit_pe`, `probs`, `fy26e_npat` untouched — judgment, human-only. | `assumptions.json` → `tcx.actuals._THE_CORRECTION_MOVES_A_KILL_CRITERION_FROM_COMFORTABLE_TO_NEARLY_ARMED_2026_08_02`; log 02-Aug 09:53 |
...
| 6 | **Decide whether to add a `spot_persists` branch to HPG.** | No existing branch describes current input prices. At spot the case is **₫0.60–0.68m/t** against a bear branch of ₫1.25m. **Better evidenced 31-Jul, and correctly dated:** HPG's own August HRC offer is US$546–547/t list and **US$535/t volume**, bracketing the model's US$539 spot almost exactly — so the spot input is sound and the August cut is already inside it. **But Q3 straddles the step** (July ≈580 → August ≈546), so a Q3 average sits *above* spot. **This branch describes Q4, not Q3** — a soft Q3 print is not its vindication. **Repriced 31-Jul 20:53 with all three inputs pinned** (ore US$98.25, coal US$218.50, HRC US$546.5 list / US$535 volume): the branch is now **₫0.86–1.10m/t**, not the ₫0.36m/t the mis-logged ore implied nor the ₫0.83m/t of 27-Jul. **Still below the ₫1.25m bear branch on both readings**, so the finding stands — but the gap is ₫0.15–0.39m/t, and closing. | `hpg-spread-bridge.md` §6–7 |
