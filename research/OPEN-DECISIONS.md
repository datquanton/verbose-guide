...
| 25 | **⚠⚠ Settle TCX's `pe_ttm` / `npat_ttm`, then re-check `cap_now` on all eight. ← NEW, escalated 02-Aug 10:53. ESCALATION TRIGGER 3 fired at more than 3× its threshold. Largest single-name error found so far, and it is in the ENGINE, not the narrative.** | **`decide.py` line 67 computes `cap_now = pe_ttm × npat_ttm`** and every expected return is `(exit_pe × fy26e_npat) / cap_now − 1`. For TCX that is **20.3 × 4,050 = ₫82,215bn** against a true market cap of **2,773,896,000 × ₫41,100 = ₫114,007bn** — **38.7% too low**, so every branch return is overstated. **mu_raw goes from +23.1% to −11.2% at the file price (−6.9% at the verified 24-Jul price): a 34.3pp move that CHANGES SIGN.** **TCX is ranked FIRST and is a proposed ADD; on the corrected market cap its expected return is negative.** **Which input is wrong is not established** — either `pe_ttm` should be 28.15 or `npat_ttm` should be ₫5,616bn, or both are stale from before the stock dividend — **so neither was changed** (charter §5 forbids retuning an unresolved measure; §4 scope-locks `decide.py`). **Confidence is the wrong instrument here**: it multiplies `mu_raw`, so shrinking it scales a wrong number. **The fix is one read — TCBS's TTM NPAT off the filed statements, which also settles `pe_ttm`.** **⚠ AND THE SAME DEFECT MAY EXIST ON THE OTHER SEVEN:** `cap_now` is `pe_ttm × npat_ttm` for every name and **the file holds no share count for any of them**, so **no ticker's market cap has ever been cross-checked against shares × price.** That check has not been run. | `assumptions.json` → `tcx.actuals._ESCALATION_TRIGGER_3_THE_ENGINE_PRICES_TCX_OFF_A_MARKET_CAP_38_7_PCT_TOO_LOW_2026_08_02`; log 02-Aug 10:53 |
