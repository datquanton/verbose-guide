...
| 25 | **⚠⚠⚠ Settle `pe_ttm` / `npat_ttm` on SEVEN names — `cap_now` is wrong on seven of eight and SIX FIRE TRIGGER 3, at 48.2% of the book. ← opened 10:53, widened 11:53, completed 13:53. Largest defect found, and it REORDERS the ranking.** | **`decide.py` line 67 sets `cap_now = pe_ttm × npat_ttm`** and divides every scenario by it. **Implied vs actual share count:** TCB 7,065.3m vs 7,086.2m (**−0.3%, the only correct name**); KDH 1,000.5m vs 1,122.1m (−10.8%, **−12.6pp ⚠**); HPG 7,800.3m vs 8,443.0m (−7.6%, −8.8pp, **below threshold**); VPB 6,739.4m vs 7,933.9m (−15.1%, **−15.3pp ⚠**); MBB 9,495.0m vs 8,055.0m (**+17.9%**, **+20.7pp ⚠**); TCX 2,000.4m vs 2,773.9m (−27.9%, **−34.3pp ⚠**); VCI 922.9m vs 1,152.2m (−19.9%, **−22.0pp ⚠**); VPX 1,473.7m vs 1,875.0m (−21.4%, **−27.2pp ⚠**). **MBB is the one that goes the OTHER way — its 15% dividend has NOT executed (record date 12-AUG-2026), so `cap_now` is too HIGH and correcting it RAISES MBB's return. That SUPPORTS the MBB add and destroys the TCX add.** **It also settles item 19's puzzle: 16.0% × 0.55 = the recorded +8.8% exactly; corrected +20.2% — but the branch re-derivation is still needed.** **VPX verified to the dong** (1,875m × ₫25,400 = the stated ₫47,625bn). **⚠ THE RANKING REORDERS:** engine VPX / TCX / KDH / MBB / HPG / VCI / TCB / VPB → corrected **MBB +36.7 / HPG +6.4 / KDH +3.5 / TCB +1.6 / VPX −0.0 / TCX −11.2 / VCI −11.3 / VPB −13.9**. **The engine's top two become fifth and sixth; four names turn negative.** **Robustness:** share counts are firm on all eight; directions are robust because every gap but TCB's and HPG's is 10.8–27.9% against ~5% price uncertainty; **HPG is the one name where price error could matter, and it is also the only sub-threshold one.** **Nothing retuned, no confidence moved** (§5, §4). **⚠ NO WEIGHT SHOULD MOVE ON THE CURRENT RANKING — nor toward the corrected one, which rests on prices already known stale.** **Sequence: (1) refresh eight dated prices [item 2], (2) settle `pe_ttm`/`npat_ttm` per name, (3) THEN re-rank.** | `assumptions.json` → `_meta._CAP_NOW_AUDIT_COMPLETE_ALL_EIGHT_SIX_FIRE_AND_THE_RANKING_REORDERS_2026_08_02_1353`; log 02-Aug 10:53 → 13:53 |
