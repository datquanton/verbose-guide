...
| 25 | **⚠⚠ Settle `pe_ttm` / `npat_ttm` on FIVE names — the `cap_now` defect is BOOK-WIDE. ← escalated 02-Aug 10:53 on TCX, WIDENED 11:53 after running the cross-check. ESCALATION TRIGGER 3 fires on TWO names. Largest defect found so far, and it is in the ENGINE, not the narrative.** | **`decide.py` line 67 sets `cap_now = pe_ttm × npat_ttm` for all eight**, and every expected return is `(exit_pe × fy26e_npat) / cap_now − 1`. Market cap is shares × price, so `cap_now` is right only if the implied share count is current. Shares from charter capital at ₫10,000 par. **`TCB` (35%) is CORRECT to 0.3% — the largest position is unaffected, and that is the most important line here.** **⚠ `VPB` (10%) fires: `cap_now` −15.1%, mu_raw +1.4% → −13.9%, a 15.3pp move.** Its implied count is **6,739.4m = charter ₫67,394bn, VPB's ~2022 figure** against a current ₫79,339bn — **~3 years stale, and the direction is robust because every 2026 action only raises the count.** **⚠ `TCX` (5.5%) fires: −27.9%, +23.1% → −11.2%, a 34.3pp move.** **`HPG` (16.8%) is material but does NOT fire at −8.8pp** — its inputs predate the **767m-share issue that settled in May 2026** (charter ₫76,755bn → ₫84,430bn). **Direction NOT established on two: `MBB` (6.5%)** — +17.9% too high on the pre-dividend count (+20.7pp) but only +2.5% if the 15% dividend executed (+2.9pp); **turns on the ex-date and whether ₫22,050 is adjusted.** If the larger reading holds it explains item 19 exactly (16.0% × 0.55 = the recorded +8.8%; corrected would be +20.2%) — **a hypothesis, not a finding, and not to be used to argue the MBB add.** **`VCI` (3.1%)** — −22.0pp if the ESOP+bonus completed, the other way if not. **Not checked: KDH (20.3%), VPX (2.8%).** **THE ERRORS DO NOT SHARE A DIRECTION**, so they do not cancel in the ranking. **Nothing retuned anywhere** (§5 unresolved measure, §4 scope-lock), and **confidence deliberately not moved on any name** — it multiplies `mu_raw`, so it would disguise a denominator fault as a conviction change. **Fix: one read per name — current shares outstanding, or TTM NPAT off the filed statements.** | `assumptions.json` → `_meta._THE_CAP_NOW_DEFECT_IS_BOOK_WIDE_NOT_A_TCX_PROBLEM_2026_08_02_1153`; log 02-Aug 11:53 |
