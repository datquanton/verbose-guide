...
| 25 | **⚠⚠ Settle `pe_ttm` / `npat_ttm` — the `cap_now` defect is BOOK-WIDE and THREE names fire trigger 3. ← opened 02-Aug 10:53 on TCX, widened 11:53, COMPLETED 12:53 on seven of eight. Largest defect found so far, and it is in the ENGINE.** | **`decide.py` line 67 sets `cap_now = pe_ttm × npat_ttm` for all eight** and divides every scenario by it. Shares from charter capital at ₫10,000 par. **⚠ THREE FIRE, 35.8% of the book:** **`KDH` (20.3%)** −10.8%, **−12.6pp** — confirmed floor 1,011.1m shares (already above the implied 1,000.5m), likely 1,122.1m after the 111m 2025 issue; **fires on the likely reading, not established beyond doubt — one completion notice settles it.** **`VPB` (10.0%)** −15.1%, **−15.3pp** — implied 6,739.4m = charter **₫67,394bn, a ~2022 figure**; the 26% issuance is ~2.07bn shares to ₫100,000bn with the **first tranche in implementation May 2026**, so it is stale on any reading. **`TCX` (5.5%)** −27.9%, **−34.3pp**. **`TCB` (35.0%) is CORRECT to 0.3% — the largest position is unaffected, and that is the most useful line here.** **`HPG` (16.8%)** −7.6%, −8.8pp — **below the 10pp threshold, does NOT fire**, but its inputs predate the 767m-share issue that settled May 2026. **Direction NOT established: `MBB` (6.5%)** (+17.9% and +20.7pp on the pre-dividend count, +2.5% and +2.9pp if the 15% executed — turns on the ex-date and whether ₫22,050 is adjusted; **if** the larger reading holds it explains item 19 exactly, 16.0% × 0.55 = the recorded +8.8% vs +20.2% corrected — **a hypothesis, not usable to argue the MBB add**) and **`VCI` (3.1%)**. **`VPX` (2.8%) NOT CHECKED and not claimed clean.** **THE DIRECTIONS DO NOT AGREE** — four understate market cap, MBB probably overstates, TCB is right — so **correcting them REORDERS the ranking rather than shifting it uniformly.** **Nothing retuned; no confidence moved on any name** (§5 unresolved measure, §4 scope-lock; confidence multiplies `mu_raw` and would disguise a denominator fault as a conviction change). **Fix: one read per name — current shares outstanding, or TTM NPAT off the filed statements.** **NO WEIGHT SHOULD MOVE ON THE CURRENT RANKING.** | `assumptions.json` → `_meta._CAP_NOW_AUDIT_COMPLETE_EXCEPT_VPX_THREE_NAMES_FIRE_2026_08_02_1253`; log 02-Aug 11:53 and 12:53 |
