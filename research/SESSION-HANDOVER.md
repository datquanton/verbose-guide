...
> **⚠ CURRENT STATE — Thu 13-Aug-2026 10:54 ICT. Read this block first; everything below it is older.**
>
> **MARKET.** VN-Index closed **1,793.18** on Wed 12-Aug, **+19.77 pts / +1.11%**, on materially lower
> turnover, crossing above its 200-day MA. It reconciles exactly to the 11-Aug close of **1,773.41**
> (1,773.41 + 19.77 = 1,793.18). **Foreigners net BOUGHT on 12-Aug — direction only, no magnitude reported** —
> reversing 11-Aug's **₫768.77bn** net sell in which **TCB was the most-sold stock on the exchange at
> ₫221.8bn**. TCB's foreign room is on file as LOCKED at 22.4908% and EXHAUSTED.
>
> **US JULY CPI (12-Aug, 19:30 ICT).** Headline **+0.1% MoM / 3.4% YoY**, core **+0.2% / 2.5%** — **4 of 4
> exact against the Dow Jones consensus**. ⚠ *An earlier "miss" I reported was a PHANTOM: my pre-registered
> table had headline MoM at +0.2%, taken from a market-calendar post and mislabelled "Dow Jones". Structurally
> the same error as the KDH phantom — a real calculation against a referent that was not there.* **CME
> September rate-HIKE odds 42%.** US session: S&P +0.26%, 2Y and 10Y both −4bp, **DXY flat at +0.01%** — the
> pre-stated chain's Fed leg delivered and **its USD leg did not**.
>
> **MBB.** Record date **12-Aug** landed exactly as gate row 43 predicted (ex 11-Aug, last cum 10-Aug),
> derived from MBB's own July precedent. Offering runs under SSC Certificate **335/GCN-UBCK, 31-Jul-2026**.
> Dated tranche is **+25.0%** (15% stock dividend >1.2bn shares + 805.5m rights at ₫10,000); the file's
> **+27.5%** additionally counts the undated ≤200m placement. **Rights transferable 18–26 Aug (~₫992),
> subscription to 07-Sep.** `valuation.MBB.price` still **₫22,050** against a worked adjustment of **₫18,440**
> — §4 human-only. **⚠ Item 38's `fy26e_npat` re-derivation was due 12-Aug and has NOT been done.**
>
> **CLOSED THIS CYCLE.** The iron-ore range-break row is **CLOSED BY ITS OWN STATED DEFAULT**: both conditions
> failed, so `iron ore` stays at **US$98.25** and the −3.87% break is never claimed. *Four "iron ore" numbers
> now circulate across sources spanning 9.7% — US$94.45 / 96.55 / 98.25 / 100–103.60 — which is why it was
> never banked.*
>
> **⚠⚠ NEW BLOCKED HOST: `bls.gov`.** Joins `lseg.com`, `vsd.vn` and five others. **Every authoritative PRIMARY
> registry this file needs is unreachable while the secondary press repeating them is reachable — T1 is
> effectively unattainable here for regulatory and statistical facts.** The July CPI figures are **T4
> permanently**; that re-confirmation instruction is VOID, not pending.
>
> **⚠ TWO SELF-AUTHORED UNOBSERVABLE CONDITIONS IN ONE DAY** (iron ore 02:54, bls.gov 20:54) — the seventh and
> eighth catalogued. *Mechanism: both were written at the END of an entry as a closing gesture, after the
> checking energy had gone into the finding. The follow-up line is where discipline is cheapest to skip.*
>
> **DATED AHEAD.** 15-Aug SBV circular cluster (**29/2026/TT-NHNN**, already on file) · 18–26 Aug MBB rights
> trading · **21-Aug: FTSE constituent list AND KDH insider-window close** · 24-Aug CORE circumvention
> preliminary · 02-Sep VCI window close · mid-to-late Sep rebar ITC vote · 21-Sep FTSE effective.
>
> **STANDING.** No model input has changed in this cycle; `DECISION-BRIEF.md` is byte-identical throughout and
> **no escalation trigger has fired**. **FiinQuant MCP is still unauthorised** — it is the only route to
> single-stock closes and to reopening the iron-ore row. The parallel audit session is **NOT reachable**
> (`ListAgents`: no reachable agents); any findings are on `claude/vn-research-parallel-thread`.
