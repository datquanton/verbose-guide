...
**Updated 07-Aug 18:53 ICT.** *(18:53 — **⚠⚠ enumerated the forecast register and there ISN'T ONE. Two live forecasts sit off the scoreboard, on the file the repo calls its most important.** `calibration-log.md` opens *"every forecast that enters `assumptions.json` gets scored here"* — **run as an enumeration rather than taken on trust.** **Referenced anywhere: #1, #2, #4, #7, #8, #10 (+#9, mentioned once inside the log and never registered elsewhere). Scored: #1, #2, #4, #10 — MISS, MISS, MISS, HIT.** **⚠⚠ #7 and #8 are referenced 5× and 3× in `assumptions.json` and appeared ZERO times in the calibration log:** **#7 — TCX FTSE event, +20%/+5%/−15%, EV +6.25%, resolves ~21-Oct (75 days, 5.5% position); #8 — CAEX licence ~Q3 at p=0.40, resolves by 30-Sep (54 days, VPX).** *The file already knew about #8 — a 05-Aug ticker-block entry calls it "open and unscored, 56 days left", and 05-Aug + 56 = 30-Sep exactly. **It knew in the ticker block; the scoreboard did not.*** **⚠ And #3, #5, #6 exist NOWHERE** — the numbering is not contiguous and nothing says why, **because IDs were minted in prose and scored in prose.** **An edge is a hit rate, and a hit rate over a set you cannot enumerate is not a measurement:** the board reads **1 hit / 3 misses**, with **two open forecasts outside the denominator, both resolving inside eleven weeks.** **ACTION: a dated FORECAST REGISTER appended to `calibration-log.md`** — additive and in scope (that file is not §4-locked). **NOT done: nothing scored (nothing resolved), no probability re-weighted, no ID invented.** *Re-weighting #7 now on FTSE-mechanics evidence is the post-hoc adjustment pre-registration exists to prevent — the file says so itself.* **Nothing modelled; no confidence moved; no trigger fires.** *Payrolls at 19:30 ICT — readable next sweep.*)*
