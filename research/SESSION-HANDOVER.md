...
**Updated 07-Aug 21:53 ICT.** *(21:53 — **⚠⚠ VPB's foreign placement has a PRICING FLOOR, which partly decides a criterion I filed as "conditional" six hours ago — and the file cannot finish the job.** **Structure: Phase 1 the 26% stock dividend (₫79,339bn → ₫100,000bn, Q2–Q3); Phase 2 a placement of >624m shares to ONE foreign investor → ~₫106,244bn, expected Q3–Q4. Foreign ownership could reach 34%**, above the standard 30% bank ceiling. **⚠ THE FLOOR: the approved plan prices it "not below book value" per the consolidated statements of the prior quarter — the issuer has bound its own downside.** **So the armed test *"executed >15% below market"* reduces to one comparison: on the dated ₫25,150 a 15% discount is ₫21,378, so it can only fire if BVPS < ₫21,378 — equity below ₫169,607bn — and only if the placement prices AT the floor.** *"Market" is still undefined; that half stands.* **⚠ AND THE FILE HOLDS NO EQUITY FIGURE FOR VPB — `equity` exists for TCX alone (₫45,782bn)** — and press cannot supply it, only the statements can. **⚠⚠ SO ITEM 37 SHARPENS FROM TWO TO THREE: the 29-Aug consolidated reviewed statements unblock FE Credit's NPL and loan balances (coverage gap + item 36's criterion) AND VPB's shareholders' equity (this criterion). ONE STATEMENT, THREE UNBLOCKS**, on a 10.0% position with two armed criteria that cannot currently be run. *Smaller datum, derived and flagged: Q2 consolidated PBT ~₫11,000bn against H1 ₫18,880bn implies Q1 ≈₫7,880bn, Q2/Q1 = 1.40× — the file holds no quarterly split, and "gần 11.000" is rounded.* **Nothing modelled; no confidence moved; no trigger fires — the placement has not priced. The 14:53 trigger-3 and 06-Aug trigger-5 escalations stand.**)*
