...
**Updated 06-Aug 18:53 ICT.** *(18:53 — **⚠⚠⚠ ESCALATION TRIGGER 5 IS LIVE WITH A TWO-SESSION FUSE.** **MBB ex-rights 11-Aug-2026 · record date 12-Aug · rights subscription 18-Aug–07-Sep · charter capital above ₫100,600bn.** The date had stood unestablished since 04-Aug because `vsd.vn` returns `connect_rejected`; an ordinary corporate-action search produced it and **the file's own capital plan verified it to the decimal** (1,208.25m · 805.5m · 80,550 + 12,082.5 + 8,055 = ₫100,687.5bn). **It lands on the brief's #1 name and largest add — BUY MBB 6.5% → 13.3% — three sessions out, against the brief's own rule that nothing should fight a dated catalyst inside five. Last cum session: Mon 10-Aug.** **THE DENOMINATOR IS THE EXPOSURE:** `cap_now` = ₫22,050 × 8,055m = ₫177,613bn and **both fields are human-only under §4 and both change on 11-Aug** — the stock dividend is cap-neutral, the **10:1 rights at ₫10,000 is not** (₫8,055bn injected, theoretical ex-all **₫18,440, −16.4%, which is not a loss**). **Refresh one field without the other and cap_now is wrong by up to 25% on the position the engine wants to double.** **E[r], computed and deliberately not overstated:** the engine was first reproduced exactly (+36.69% / +18.34% vs the brief's +36.6 / +18.3), then re-run post-rights at **+30.76% / +15.38% — −5.93pp raw, −2.97pp shrunk, which does NOT clear the 10pp threshold, so trigger 3 does NOT fire.** **But the bear branch flips +2.47% → −1.98%** and the largest add loses the thin positive bear branch it rests on. **⚠ That drop is a MODEL ARTEFACT, not a forecast** — `exit_pe × fy26e_npat` prices 2026 earnings while the cash enters `cap_now` in September; a ₫10,000 rights price against a ₫22,050 market hurts **non-subscribers** and is near-neutral for a subscriber. **The brief's standing instruction — *fy26e_npat REQUIRES HUMAN RE-DERIVATION* — finally has a deadline: 10-Aug.** **A cash call, not a trade:** ₫10,000 per right, one per ten shares at 12-Aug. **Nothing modelled; every field involved is guarded. This system recommends; a human signs.** New item 38.)*
