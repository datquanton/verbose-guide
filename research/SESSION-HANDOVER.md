...
**Updated 07-Aug 17:53 ICT.** *(17:53 — **the steel peers tested HPG's spread thesis from OUTSIDE and ruled out the reading that would have been bad for it.** **The gap first: `NKG`, `Nam Kim`, `Hoa Sen`, `HSG` returned exactly ONE hit in the file** — a passing product-line note — **so the two listed companies that BUY HPG's output had no results and no margins on file**, on a 16.8% position whose thesis is a spread. **Q2: NKG NPAT ₫102.6bn (from a small loss), gross margin 7.1% → 10.5% (+3.40pp); HSG NPAT ~₫382.3bn (+39.6%), gross margin 14.86% vs an implied 12.43% (+2.43pp); HPG core NPAT/tonne ₫1.682m → ₫1.803m (+7.2%).** **⚠ THE POINT IS WHAT IT RULES OUT: NKG and HSG BUY HRC, HPG SELLS it — if HPG's spread gain had come from pricing into its own customers, their margins would have COMPRESSED. Both expanded. The improvement is chain-wide, not zero-sum.** That points at the raw-material end, where the file already has the leg — **coking coal US$228 FOB, down an eighth week; ore ~flat at 93.66.** *Limit stated: the mechanism is inferred, the simultaneity is established — and the negative claim is the valuable one, because the zero-sum reading is the one that would have made HPG's Q2 borrowed and reversible.* **Street context, not scored: VCBS forecast HPG Q2 growth +25% with both peers "hụt hơi"; actual +50.9%, 2.04× the forecast, and both peers grew.** *Second independent line today on the same armed criterion, which was run to a verdict and PASSED at 07:53.* **Nothing modelled; `core_npat_per_tonne_m` untouched (§4); no confidence moved; no trigger fires.** *Next: **US July payrolls 19:30 ICT** — the 05-Aug pre-registration's direct test. **MBB's last cum session is Monday 10-Aug.***)*
