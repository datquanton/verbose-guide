...
**Updated 07-Aug 13:53 ICT.** *(13:53 — **⚠⚠ A HARD DATED PRICE ON THE 35% POSITION, and item 3 stops being a convention argument.** **TCB closed 03-Aug-2026 at ₫29,950, market cap ₫212,232.9bn** — the two divide to **7,086,240,401 shares against the file's 7,086,200,000, +0.0006%**, the tightest external share-count confirmation in the book. *(Par guard passed; the file's own cap/price ratio matches to five decimals.)* **The file is 2.34% below.** **Marked:** raw **+3.98% → +1.60%** (−2.37pp), shrunk **+2.78% → +1.12%** — **below the 10pp threshold, trigger 3 does NOT fire.** *(Engine reproduced exactly first: +3.98/+2.78 vs the brief's +4.0/+2.8.)* **⚠⚠ AND THE REPRODUCTION SETTLES ITEM 3.** TCB is the **only** name with a non-zero `cash_yield`, so this was the first test of the field: **ex-cash_yield +1.58%, with the 0.024 +3.98% — it enters ADDITIVELY and contributes EXACTLY its own value** (probabilities sum to one). **So item 3 is now arithmetic: MBB's unrecorded 4.54% is worth EXACTLY +4.54pp of raw E[r] on the engine's #1 name and largest add; VPB's 2.00% is worth +2.00pp; TCB's 0.024 already supplies +2.40pp — 60% of its entire +3.98%.** **⚠ THE DIRECTIONS FIGHT: TCB's price marks DOWN 2.37pp while MBB's cash_yield marks UP 4.54pp — the two open input defects move the ranking's top and its largest weight opposite ways, so neither is "small".** **ITEM 2 TALLY: four of eight measured, 61.2% of the book — TCB −2.34% · KDH +1.1% · VCI bracketed · VPX 12-Jun. Every gap under 2.5%, and they do NOT share a sign** — third straight hour contradicting the 01:53 uniform +4.63% premise. *Unmeasured: HPG 16.8%, VPB 10.0%, MBB 6.5%, TCX 5.5%.* **Nothing modelled; `price` and `cash_yield` untouched; no confidence moved.**)*
