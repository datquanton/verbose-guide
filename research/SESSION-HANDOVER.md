...
**Updated 07-Aug 01:53 ICT.** *(01:53 — **OPEN ITEM 2 finally gets a size and it is trigger-3 scale on the engine's #1 name.** **Status check first, because the phrase recurs in every summary:** the `_meta` blocks' *"SIX FIRE TRIGGER 3, AT 48.2% OF THE BOOK"* is from 02-Aug and was **resolved the same day** by the 20:53 refresh — **no standing fired triggers exist**, so the recent "no trigger fires" closings are accurate. **But that note left one thing live and it has aged 14 days:** *prices are presumed 24-Jul*, and item 2 has stood since as *"the single remaining input standing between this file and a trustworthy ranking"* **with no magnitude attached.** **It has one now.** The file's own market context closes the VN-Index at **1,686** in the week of 20–24 Jul against **~1,764 on 06-Aug — +4.63%.** **IF** the eight moved with the index, every `cap_now` is understated ~4.6%; on MBB raw **+36.69% → +30.64%**, and **with the 11-Aug rights, → +25.22%, a −11.47pp raw move that WOULD clear the 10pp threshold.** **⚠ NOT declared fired** — it assumes prices really are 24-Jul (*presumed*, which is why the item is open) and that MBB runs at **beta 1**, and applying an index return to a single stock is the wrong-quantity error caught three times this week. **What is established is that item 2 is worth potentially >10pp on the largest position in the brief — not housekeeping.** **⚠⚠ AND AFTER 11-AUG THE STALE PRICE BECOMES A TRAP:** refreshing MBB post-ex (theoretical **₫18,440**) against the unrefreshed **8,055m** count gives cap_now **₫148,534bn vs a true ₫185,668bn — 20.0% too low, inflating E[r] ~25%.** **Shares first, price second; neither is modifiable here (§4).** **SECOND: VPB's ex-date is not unknown, it is UNSET** — the 26.04% issue (charter ₫79,339bn → ₫100,000bn, checks to 2,065.99m vs the file's 2,060m) is slated for **Q3–Q4 pending regulatory formalities**. **VPB's and TCB's are pending approval, not pending discovery, so neither can surprise the book in the next few sessions.** *Item 3's tax treatment confirmed: VPBank's July written consultation changed the plan to a stock dividend, exactly as the file classifies it.* **Nothing modelled; no confidence moved; no trigger declared fired.**)*
