...
**Updated 07-Aug 23:53 ICT.** *(23:53 — **⚠⚠⚠ NOTHING AN AUTOMATED SWEEP FINDS CAN REACH `DECISION-BRIEF.md`. Verified in `decide.py`, not inferred — a grep for uppercase prose keys returns ZERO.** **Every dated prose key this repo has written is inert to the generator:** the trigger-3 firing on MBB, the eight defective criteria, price staleness on all eight names, the KDH criterion collision, the forecast register — **all of it lives in `monitoring-log.md`, `OPEN-DECISIONS.md` and keys `decide.py` does not read.** **⚠ A BRIEF-ONLY READER TONIGHT SEES:** MBB **#1 at +36.6% / +18.3%**, *"BUY MBB 6.5% → 13.3%"*, and **all seventeen criteria "armed"** — **and does NOT see** that trigger 3 fired on that name today (−10.58pp on a dated price), that trigger 5 fired eight days ago with **the last cum session MONDAY 10-Aug**, that prices are **14 days stale on every name**, or that **9 of 17 "armed" criteria cannot be run.** **⚠ The enforcement line — *"a triggered criterion… forces the resize at the next brief"* — can only be invoked through `kill_criteria.status`, §4 human-only** (line 372 echoes it verbatim), **so the brief can only report an escalation a human already entered.** **⚠⚠ THE ONE CHANNEL, IDENTIFIED AND DELIBERATELY NOT USED: `valuation.<TICKER>.evidence` IS read and printed verbatim** (verified character-for-character on MBB), **is not §4-locked, and was not in my guard — so writing an escalation banner there was mechanically permitted, and I did not.** *`evidence` feeds a human's judgement about a position, and §5 says thesis-bearing evidence is escalated, not worked into the artefact.* **`evidence` was ADDED to this sweep's guard before writing and is verified unchanged on all eight.** **The human's question is now specified: escalations into `evidence`, or a new brief section (which needs `decide.py`, §4)?** ***The findings exist; the path from finding to decision does not, and the gap is a reading order nothing in the repo states.*** **Nothing modelled; no field touched; no NEW trigger — the 14:53 trigger-3 and 06-Aug trigger-5 stand.** New item 45.)*
