...
**Updated 07-Aug 05:53 ICT.** *(05:53 — **STRUCTURE, SEVENTH OF THE SPECIES: two of the five shortlisted Vietnamese crypto exchanges belong to this book's ecosystems and the file recorded ONE.** `TCEX` appeared **zero times** in `assumptions.json` and zero times in the log. **Size first, so nothing is inflated: for TCX this is negligible** — TCEX charter **₫101bn**, TCBS's 9.9% = **₫10bn = 0.0088% of TCX's market cap.** **The finding is the ownership shape: Nguyễn Xuân Minh, chairman of TCBS, holds 89% PERSONALLY; TCBS 9.9%; Techcom Fund Management 1.1%** — the brand, ecosystem and distribution are Techcom's, and **89% of any upside is one individual's.** **Contrast the arm the file DID price as an option: VPBankS put >₫1,097bn into CAEX for 11% — 11× TCEX's entire charter capital.** **⚠ A THIRD CHANNEL LINKING THE CLUSTERS:** TCB+TCX 40.5% and VPB+VPX 12.8% now both hold stakes in **competing exchanges in one five-slot pilot** — a single MoF decision touching both. *Open item 7 asks whether `corr_same_cluster` at 0.80 understates a parent/subsidiary pair; three independent channels have now landed on the same side of it in two days, none found by looking.* **⚠ AND IT BEARS ON VPX'S ARMED CRITERION** — *"CAEX licence rejected under the 5-licence cap"*: **MoF took 7, shortlisted 5 (CAEX, VIXEX, LPEX, TCEX, Vietnam Digital Assets) and eliminated 2, so the field was cut by SCREENING, not the cap** — same defect species as MBB's MBV criterion. **Shortlisted ≠ licensed** (TCEX's "round 1" implies a round 2), **the shortlist's date is NOT established**, and *"5 of 7 consulted"* vs *"the cap is filled"* are **different claims — the second is an inference, named not asserted.** **Flagged for the human: a 15% rejection-under-the-cap branch looks high if CAEX is one of five for five slots.** **Nothing modelled; no confidence moved; no trigger fires.** New item 42.)*
