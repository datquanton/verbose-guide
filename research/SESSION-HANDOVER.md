...
**Updated 07-Aug 09:53 ICT.** *(09:53 — **went to close one of the four CONDITIONAL criteria and it got WORSE.** **Dung Quất 2's nameplate is 5.6mt HRC/yr** — solid, including HPG's own site, plus an approved +500,000t — so yesterday's *"nameplate not on file"* caveat is gone. **⚠ And that is when the real defect shows: the condition names *DQ2* while the disclosed figure is *group* HRC volume** (`q2_hrc_volume_mt` 1.9). **Subject/measure mismatch — third of that species** after MBB's MBV drag and VCI's foreign-vs-total share. **Q2 1.9mt → 7.6mt annualised: 135.7% of DQ2 alone (meaningless, it includes DQ1) or 84.4% of a ~9mt total — and the ~9mt is SOFT** (*"HRC **and high-quality steel**"*; DQ1's line only by subtraction). **The solid number answers a question the criterion doesn't ask; the shaky one answers the question it does.** **AUDIT REVISED DOWNWARD: 5 testable · 9 NOT testable · 3 conditional — not-testable is now 53%, a majority, against 47% eighteen hours ago.** **⚠ Worth the human's eye regardless: the group-HRC run-rate already sits ON the threshold at 84.4% — but that is a Q2 figure against a Q4 test in a plant still ramping (DQ2's second blast furnace first cast Sept-2025), so it is NOT a warning; it means the Q4 test is live rather than academic, and the denominator that decides it is undefined.** *One search supplied the data and the criterion still cannot be run, because the missing thing was never the number.* **Nothing modelled; no criterion status changed; no confidence moved; no trigger fires.**)*
