...
RESERVE = (18925.5, 20524.5, 22305.4)           # Model!Y/Z/AA286 after the overlay
COV = tuple(r / (n * l) * 100 for r, n, l in zip(RESERVE, NPLR, LOANS))

ROWS = [
    ('', 'STB — REVISION 05/08/2026 (tăng dự phòng FY27F, FY28F)', '', '', '', '', ''),
    ('A8', 'Trích thêm dự phòng FY27F và FY28F', '1,000 / 1,000', 'VNDbn', 'MAS',
     'Model!Z278 = -Z279+1000, AA278 = -AA279+1000 (trước đó = -Z279*1, -AA279*1)',
     ...
     'mà bồi đắp bộ đệm: dự phòng đã trích tăng 1,000 tỷ năm FY27F và 2,000 tỷ lũy kế FY28F'),
    ('A9', 'Chi phí dự phòng FY26F / FY27F / FY28F',
     '%.0f / %.0f / %.0f' % U.PROV, 'VNDbn', 'MODEL', 'Model!Y/Z/AA141',
     'FY27F từ 9,636 lên 10,636; FY28F từ 6,982 lên 7,982'),
    ('A10', 'LNTT FY27F / FY28F', '%.0f / %.0f' % U.PBT[1:], 'VNDbn', 'MODEL',
     'Model!Z144 / AA144',
     'FY27F từ 10,872 xuống %.0f (+%.1f%% CK, trước là +45.7%%); FY28F từ 17,271 xuống '
     '%.0f (+%.1f%% CK, trước là +58.9%%). Nền FY27F thấp hơn nên tăng trưởng FY28F cao lên'
    ...
    ('A11', 'LNST / ROE FY27F / FY28F',
     ...
     '%.0f tỷ, thấp hơn 1,557 tỷ so với bản trước do lợi nhuận giữ lại giảm'
    ...
    ('G22', 'Bao phủ nợ xấu FY28F chạm ~100%',
     ...
     'Trước khi trích thêm là 50.9 / 67.8 / 91.1%. Bao phủ 100% ở một ngân hàng vừa '
     'xử lý xong tồn đọng là mức rất cao — CV cân nhắc có muốn giữ hay không'),
